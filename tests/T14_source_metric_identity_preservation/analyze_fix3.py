...
RAW_SRC=HERE/"raw"/"generation_args_typed_trace_fix3.jsonl"
RAW_SUB=HERE/"raw"/"source_id_subgroup_results_fix3.jsonl"
RAW_GROUP=HERE/"raw"/"production_group_counterfactuals_fix3.jsonl"
RAW_ERR=HERE/"raw"/"integrity_errors_fix3.jsonl"
SUMMARY=HERE/"results"/"summary_fix3.json"
ANALYSIS=HERE/"results"/"RESULT_ANALYSIS_FIX3.md"
...
def fail(msg):
    SUMMARY.write_text(
        json.dumps({"test_id":"T14_source_metric_identity_preservation","fix_id":"HARNESS_FIX_03","verdict":"ERROR","error":msg},
                   indent=2,sort_keys=True)+"\n",
        encoding="utf-8"
    )
    print("T14 FIX3 ERROR")
    print(msg)
    raise SystemExit(2)
...
if not (HERE/"raw"/"preflight_fix3.json").exists():
    fail("Missing Fix3 preflight evidence.")
...
try:
    ...
    fail(f"cannot import frozen production/schema code: {type(e).__name__}: {e}")
...
def load_typed_record(rel):
    if rel in typed_cache:
        return typed_cache[rel]
    p=EEE/rel
    if not p.is_file():
    ...
    # This mirrors Stage A's typed Arrow cast, including schema-added nulls.
    table=pa.Table.from_pylist([padded],schema=base_schema)
    typed=table.to_pylist()[0]
    typed_cache[rel]=typed
    return typed
...
with RAW_SRC.open("w",encoding="utf-8",newline="\n") as f:
    for r in src:
        fid=r["fact_id"]
        try:
            typed=load_typed_record(r["source_record_path"])
            idx=int(r["result_idx"])
            ers=typed.get("evaluation_results")
            ...
            if not isinstance(ga,dict):
                raise TypeError("typed generation_args is not object")
            ...
            counts["typed_generation_args_rows_complete"]+=1
            f.write(json.dumps({
                "fact_id":fid,
                "comparability_group_id":r["comparability_group_id"],
                "generation_args":ga,
            ...
            counts["typed_generation_args_errors"]+=1
            errors.append({"type":"typed_generation_args_error","fact_id":fid,"error":f"{type(e).__name__}: {e}"})
...
for r in stage:
    ...
    rr["generation_args_typed"]=s["generation_args"]
...
for gid,rs in sorted(by_gid.items()):
    ...
    rows_in=[{
        ...
        "score":r["score"],
        "generation_args":deepcopy(r["generation_args_typed"]),
...
integrity_bad=(
    len(errors)>0
    or counts["typed_generation_args_errors"]>0
...
summary={
 ...
 "typed_generation_args_rows_complete":counts["typed_generation_args_rows_complete"],
 "typed_generation_args_errors":counts["typed_generation_args_errors"],
 ...
 "limitations":[
   "Fix 3 repairs typed source reconstruction after Fix 2 representation mismatch.",
 ...
 "evidence":{
   ...
   "generation_args_typed_trace_sha256":sha256(RAW_SRC),
...
for k in [
 ...
 "typed_generation_args_rows_complete","typed_generation_args_errors",
...
if verdict=="CONFIRMED":
    lines += [
      ...
      "",
      "Both complete production groups replay exactly through the frozen typed source representation before the counterfactual is interpreted.",
      "",
      "The result therefore supports operational dependence on fallback source-id collapse under the preregistered exact-source-id preservation intervention."
    ]
...
print(r"summary=tests\T14_source_metric_identity_preservation\results\summary_fix3.json")
print(r"analysis=tests\T14_source_metric_identity_preservation\results\RESULT_ANALYSIS_FIX3.md")
