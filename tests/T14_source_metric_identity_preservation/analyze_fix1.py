...
RAW_SRC=HERE/"raw"/"generation_args_source_trace_fix1.jsonl"
RAW_SUB=HERE/"raw"/"source_id_subgroup_results_fix1.jsonl"
RAW_GROUP=HERE/"raw"/"production_group_counterfactuals_fix1.jsonl"
RAW_ERR=HERE/"raw"/"integrity_errors_fix1.jsonl"
SUMMARY=HERE/"results"/"summary_fix1.json"
ANALYSIS=HERE/"results"/"RESULT_ANALYSIS_FIX1.md"
...
def fail(msg):
    SUMMARY.write_text(
        json.dumps({"test_id":"T14_source_metric_identity_preservation","fix_id":"HARNESS_FIX_01","verdict":"ERROR","error":msg},
                   indent=2,sort_keys=True)+"\n",
        encoding="utf-8"
    )
    print("T14 FIX1 ERROR")
    print(msg)
    raise SystemExit(2)
...
if not (HERE/"raw"/"preflight_fix1.json").exists():
    fail("Missing fix1 preflight evidence.")
...
# Load generation args from frozen source pointers.
errors=[]
counts=Counter()
source_by_fact={}
cache={}
...
for r in stage:
    ...
    rr.update({
        ...
        "source_metric_config":s["source_metric_config"],
        "generation_args_source":s["generation_args"],
    })
...
for gid,rs in sorted(by_gid.items()):
    ...
    for k in [
        ...
        if len(vals)!=1:
            ...
            errors.append({"type":"production_field_not_constant","group_id":gid,"field":k,"values":sorted(vals)})
        else:
            fields[k]=rs[0][k]
    ...
    cfg={k:fields.get(k) for k in ["metric_kind","metric_unit","min_score","max_score"]}
    ...
    if out is None:
        ...
        ok &= out.get("has_variant_divergence")==fields.get("has_variant_divergence")
        ok &= float_equal(out.get("divergence_magnitude"),fields.get("variant_divergence_magnitude"))
        ok &= float_equal(out.get("threshold_used"),fields.get("variant_divergence_threshold"))
        ok &= out.get("threshold_basis")==fields.get("variant_threshold_basis")
        ok &= canon(out.get("differing_setup_fields"))==canon(fields.get("variant_differing_fields"))
    ...
    if not ok:
        ...
        errors.append({
            ...
            "replay":out,
            "frozen":{
                "has_variant_divergence":fields.get("has_variant_divergence"),
                "divergence_magnitude":fields.get("variant_divergence_magnitude"),
                "threshold_used":fields.get("variant_divergence_threshold"),
                "threshold_basis":fields.get("variant_threshold_basis"),
                "differing_setup_fields":fields.get("variant_differing_fields"),
            }
        })
    else:
        counts["production_groups_replayed_exact"]+=1

# Counterfactual only if replay is exact.
subgroup_records=[]
group_records=[]
...
counts["unique_source_metric_ids"]=len({r["source_metric_id"] for r in subgroup_records}) if subgroup_records else len({s["source_metric_id"] for s in source_by_fact.values()})
...
summary={
 ...
 "limitations":[
   "Fix 1 repairs source acquisition after harness ERROR; it does not change the preregistered T14 claim.",
...
if verdict=="CONFIRMED":
    lines += [
      ...
      "",
      "Because both complete production groups replay exactly before the counterfactual, the result supports operational dependence on the fallback source-id collapse under the preregistered identity-preservation intervention.",
      "",
      "This does not establish that exact source ids are the normative canonical identity."
    ]
elif verdict=="REFUTED":
    lines += [
      "P1 was observed: both affected production-positive groups retain at least one positive exact-source-id subgroup.",
      "",
      "The positive decisions therefore cannot be attributed solely to the fallback identity collapse."
    ]
else:
    lines += [
...
print(r"summary=tests\T14_source_metric_identity_preservation\results\summary_fix1.json")
print(r"analysis=tests\T14_source_metric_identity_preservation\results\RESULT_ANALYSIS_FIX1.md")
